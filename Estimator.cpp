...
void cell_pathLengthReactionRate_estimator::score( particle* p, double path_length ) {
  tally_hist += p->wgt() * path_length * p->cellPointer()->getMaterial()->getNuclides()[0].first->getReactions()[0]->xs() * p->cellPointer()->getMaterial()->getNuclides()[0].second * p->cellPointer()->getMaterial()->atom_density();
} //gets the flux and then multiplies by the first nuclides first cross section times its atomic density for the reaction rate
  //needs to be better implemented, with individual XS search routines
